...
expresion returns[string dir, []string lv, []string lf, string cad]
    ...
    |   ref=lref ']'            {
                                    $dir = gen.NewTemp()
                                    gen.Genln($ref.aux + " = " + $ref.aux + " + "  + $ref.ecad)
...
lref returns[ string id, string aux, int dim_, string ecad ]
    :   ref=lref ',' e=expresion    {
                                        tmp1 := gen.NewTemp()
                                        gen.Genln(tmp1 + " = " + $ref.ecad + " - 1")

                                        tmp2 := gen.NewTemp()
                                        gen.Genln(tmp2 + " = " + tmp1 + " * " + entorno.GetTamDimFilas($ref.id, $ref.dim_, tope))
                                        gen.Genln($ref.aux + " = " + $ref.aux + " + " + tmp2) //suma
                                        $id = $ref.id
                                        $aux = $ref.aux
                                        $dim_ = $ref.dim_ + 1
                                        $ecad = $e.dir
